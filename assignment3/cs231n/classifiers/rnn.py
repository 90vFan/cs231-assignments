...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # affine forward (N,D) => (N,H)
        h0, cache_affine = affine_forward(features, W_proj, b_proj)
        # word embedding (N,T) => (N,T,W)
        embed_captions_in, cache_embed_in = word_embedding_forward(captions_in, W_embed)
        # RNN/LSTM       (N,T,W) => (N,T,H)
        forward_net = {'rnn': rnn_forward, 'lstm': lstm_forward}[self.cell_type]
        h, cache_rnn = forward_net(embed_captions_in, h0, Wx, Wh, b)
        # temporal affine  (N,T,H) => (N,T,V)
        ...
        # 
        backward_net = {'rnn': rnn_backward, 'lstm': lstm_backward}[self.cell_type]
    ...
    def sample(self, features, max_length=30):
        ...
        for t in range(max_length):
            if self.cell_type == 'rnn':
                ...
                h, _, _ = lstm_step_forward(x, h, c, Wx, Wh, b)
