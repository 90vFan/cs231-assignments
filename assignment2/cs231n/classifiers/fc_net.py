...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # softmax loss in layers.py
        loss, dscores = softmax_loss(scores, y)
         
        # L2 regularization for loss
        weight_reg = np.sum(np.square(self.params['W1'])) + np.sum(np.square(self.params['W2']))
        loss += 0.5 * self.reg * weight_reg
        
        # backward propagation
        da2, dw2, db2 = affine_backward(dscores, cache2)
        dx, dw1, db1 = affine_backward(da2, cache1)

        # L2 reg for gradients
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers-1:
                self.params[W_i] = np.random.randn(hidden_dims[-1], num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization == 'batchnorm':
                ...
                fc_out, fc_caches = affine_forward(out, self.params[W_i], self.params[b_i])
                bn_out, bn_caches = batchnorm_forward(fc_out, self.params[gamma_i], self.params[beta_i], self.bn_params[i])
                out, relu_caches = relu_forward(bn_out)
                caches[i+1] = (fc_caches, bn_caches,  relu_caches)
            elif self.normalization == 'layernorm':
                # affine -> layer norm -> relu
                fc_out, fc_caches = affine_forward(out, self.params[W_i], self.params[b_i])
                ln_out, ln_caches = layernorm_forward(fc_out, self.params[gamma_i], self.params[beta_i], self.ln_params[i])
                out, relu_caches = relu_forward(ln_out)
                caches[i+1] = (fc_caches, ln_caches, relu_caches)
            else:            
        ...
        # update gradients dw, db, dout in each layer
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                ...
                # dropout is the last part in forward propagation
                if self.use_dropout:
                ...
                if self.normalization == 'batchnorm':
                    ...
                    dfc, grads[gamma_i], grads[beta_i] = batchnorm_backward(dbn_out, bn_cache)
                    dout, grads[W_i], grads[b_i] = affine_backward(dfc, fc_cache) 
                    
                elif self.normalization == 'layernorm':
                    fc_cache, bn_cache, relu_cache = caches[i]
                    dbn_out = relu_backward(dout, relu_cache)
                    dfc, grads[gamma_i], grads[beta_i] = layernorm_backward(dbn_out, bn_cache)
                    dout, grads[W_i], grads[b_i] = affine_backward(dfc, fc_cache) 
